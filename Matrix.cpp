...
Matrix::Matrix(Dimension rows, Dimension columns): BasicMatrix(rows, columns) {}

Matrix::Matrix(std::initializer_list<std::initializer_list<double>> list) : BasicMatrix(list) {}

Matrix::Matrix(const BasicMatrix &other) : BasicMatrix(other) {}

BasicMatrix Matrix::getEigenvalues() const {
    if (!isSquare()) {
    ...
    if (discriminant > _epsilon) {
        ...
        return BasicMatrix({ {-a2 / 3.0 + s + t} });
    }
    // discriminant is 0
    const double alpha = cbrt(r);
    const double root1 = -a2 / 3 + 2 * alpha;
    const double root2 = -alpha - a2 / 3;
    return fabs(root1 - root2) < _epsilon ? BasicMatrix({ { root1 } }) : BasicMatrix({ {root1}, {root2} });
}
...
BasicMatrix Matrix::getNullSpace() const {
    const auto freeVariables = getFreeVariables();
    BasicMatrix result(_rows, freeVariables.size());
    auto resultColumn = 0;
...
BasicMatrix Matrix::getEigenvectorFor(const double lambda) const {
    if (!isSquare()) {
        throw std::invalid_argument("Matrix is not square");
    }

    auto beta =  Matrix(*this - identity(_rows) * lambda);
    beta.toRowEchelonForm();
    return beta.getNullSpace();
}

BasicMatrix Matrix::getEigenvectors() const {
    if (!isSquare()) {
        throw std::invalid_argument("Matrix is not square");
    }
    const auto eigenvalues = getEigenvalues();
    BasicMatrix result(_rows, _rows);
    ...
    for (Dimension eigenValueIndex = 0; eigenValueIndex < eigenvalues.rows(); eigenValueIndex++) {
        ...
        for (Dimension vectorIndex = 0; vectorIndex < eigenvectors.columns(); vectorIndex++) {
            result.setColumn(currentRow, eigenvectors.getColumn(vectorIndex));
            currentRow++;
        }
